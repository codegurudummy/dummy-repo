...
import com.amazon.campfire.presence.PresenceUtils;
...
public class PresenceUpdateHandler extends BasicModule implements ChannelHandler, ClusterEventListener {
    ...
    /** BEGIN Amazon **/
    private static final String METRIC_PRESENCE_UPDATE_HANDLER_OPERATION = "PresenceUpdateHandler.process";
    private static final String METRIC_PROCESS_EXCEPTION = "PresenceUpdateHandler.process.Exception";
    private static final String METRIC_UPDATE_CANCELED_SESSION_CLOSED = "PresenceUpdateHandler.SessionClosed";
    private static final String METRIC_BROADCAST_UPDATE_TIMER = "PresenceUpdateHandler.BroadcastUpdate";
    private static final String METRIC_BROADCAST_UNAVAILABLE_DIRECTED_TIMER = "PresenceUpdateHandler.BroadcastUnavailableDirected.Timer";
    private static final String METRIC_INIT_SESSION_TIMER = "PresenceUpdateHandler.Session.Init.Timer";
    private static final String METRIC_GET_ROSTER_TIMER = "PresenceUpdateHandler.Roster.Get.Timer";
    private static final String METRIC_GET_USER_FOR_PRESENCE_TIMER = "PresenceUpdateHandler.User.Get.Timer";
    private static final String METRIC_GET_ROSTER_FOR_SENDING_PRESENCE_TIMER = "PresenceUpdateHandler.directedPresenceSent.Roster.Timer";
    private static final String METRIC_DIRECTED_PRESENCE_ROSTER_KEEP_TRACK_TIMER = "PresenceUpdateHandler.directedPresenceSent.Roster.keepTrack.Timer";
    private static final String METRIC_PRESENCE_UPDATE_LOCK_TIMER = "PresenceUpdateHandler.directedPresenceSent.lock.Timer";
    private static final String METRIC_PRESENCE_CACHE_UPDATE_LOCK_TIMER = "PresenceUpdateHandler.directedPresenceSent.cached.lock.Timer";
    private static final String METRIC_PRESENCE_TYPE_PREFIX = "PresenceUpdateHandler.directedPresenceSent.presence.";
    private static final String METRIC_COUNT_SUFFIX = "Count";
    private static final String METRIC_TIMER_SUFFIX = "Timer";
    private static final String METRIC_UNAVAILABLE_PRESENCE_TIMER = "PresenceUpdateHandler.directedPresenceSent.presence.unavailable.Timer";
    private static final String METRIC_GET_DIRECTED_PRESENCES_CACHE_TIMER = "PresenceUpdateHandler.DirectedPresencesCache.Get.Timer";
    private static final String METRIC_PUT_DIRECTED_PRESENCES_CACHE_TIMER = "PresenceUpdateHandler.DirectedPresencesCache.Put.Timer";
    private static final String METRIC_REMOVE_DIRECTED_PRESENCES_CACHE_TIMER = "PresenceUpdateHandler.DirectedPresencesCache.Remove.Timer";
    private static final String METRIC_DIRECTED_PRESENCES_CACHE_SIZE = "PresenceUpdateHandler.DirectedPresencesCache.Size";
    ...
    public void process(Packet packet) throws UnauthorizedException, PacketException {
        ...
        final Metrics metric = OpenfireMetricsHelper.getInstance().initializeOperation(METRIC_PRESENCE_UPDATE_HANDLER_OPERATION, startTime);

        try {
            process((Presence) packet, sessionManager.getSession(packet.getFrom()), metric);
        } catch (final Exception e) {
            metric.addCount(METRIC_PROCESS_EXCEPTION, 1, Unit.ONE);
            Log.error("Presence update failed: ", e);
            throw e;
        } finally {
    ...
    private void process(Presence presence, final ClientSession session, final Metrics metric) throws UnauthorizedException, PacketException {
        try {
            ...
            if (type == null) {
                if (session != null && session.getStatus() == Session.STATUS_CLOSED) {
                    ...
                    metric.addCount(METRIC_UPDATE_CANCELED_SESSION_CLOSED, 1, Unit.ONE);
                    return;
                }
                // We track the time for this within the function.
                broadcastUpdate(presence.createCopy(), metric);
                if (session != null) {
                    session.setPresence(presence);
                    if (!session.isInitialized()) {
                        final long initSessionStartTime = System.currentTimeMillis();
                        initSession(session);
                        session.setInitialized(true);
                        metric.addTime(METRIC_INIT_SESSION_TIMER, System.currentTimeMillis() - initSessionStartTime, SI.MILLI(SI.SECOND));
                    }
                }
                ...
                final long bcastUnavailableStartTime = System.currentTimeMillis();
                broadcastUnavailableForDirectedPresences(presence);
                metric.addTime(METRIC_BROADCAST_UNAVAILABLE_DIRECTED_TIMER, System.currentTimeMillis() - bcastUnavailableStartTime, SI.MILLI(SI.SECOND));
...
// *** BEGIN amazon ***
        PresenceUtils.addUserInfo(presence, METRIC_GET_USER_FOR_PRESENCE_TIMER);
// *** END amazon ***
        return presence;
    }
     ...
     * @param metric The metrics object
     */
    private void broadcastUpdate(final Presence update, final Metrics metric) {
        ...
        if (localServer.isLocal(update.getFrom())) {
            ...
            try {
                if (name != null && !"".equals(name)) {
                    final long getRosterStart = System.currentTimeMillis();
                    Roster roster = rosterManager.getRoster(name);
                    metric.addTime(METRIC_GET_ROSTER_TIMER, System.currentTimeMillis() - getRosterStart, SI.MILLI(SI.SECOND));

                    final long bcastStart = System.currentTimeMillis();
                    roster.broadcastPresence(update, metric);
                    metric.addTime(METRIC_BROADCAST_UPDATE_TIMER, System.currentTimeMillis() - bcastStart, SI.MILLI(SI.SECOND));
                }
            }
     ...
     */
    public void directedPresenceSent(Presence update, JID handlerJID, String jid, Metrics metrics) {
        final long startTime = System.currentTimeMillis();
        if (update.getFrom() == null) {
            return;
        }
        if (localServer.isLocal(update.getFrom())) {
            ...
            if (name != null && !"".equals(name)) {
                ...
                if (!RosterManager.isRosterServiceEnabled()) {
                    ...
                    try {
                        final long getRosterStartTime = System.currentTimeMillis();
                        Roster roster = rosterManager.getRoster(name);
                        ...
                        metrics.addTime(METRIC_GET_ROSTER_FOR_SENDING_PRESENCE_TIMER, System.currentTimeMillis() - getRosterStartTime, SI.MILLI(SI.SECOND));  /** SINGLE LINE amazon ***/
                    }
                    catch (UserNotFoundException e) {
                        Log.warn("Presence being sent from unknown user " + name, e);
                    }
            ...
            if (keepTrack) {
                Log.info("Start keeping track of all the directed presences."); /** SINGLE LINE amazon ***/
                String sender = update.getFrom().toString();
                final long cacheLockStartTimer = System.currentTimeMillis();
                Lock lock = CacheFactory.getLock(sender, directedPresencesCache);
                metrics.addTime(METRIC_PRESENCE_CACHE_UPDATE_LOCK_TIMER, System.currentTimeMillis() - cacheLockStartTimer, SI.MILLI(SI.SECOND));  /** SINGLE LINE amazon ***/
                try {

                    final long lockStartTimer = System.currentTimeMillis();
                    lock.lock();
                    metrics.addTime(METRIC_PRESENCE_UPDATE_LOCK_TIMER, System.currentTimeMillis() - lockStartTimer, SI.MILLI(SI.SECOND));  /** SINGLE LINE amazon ***/

                    Log.info("Get directedPresences for sender: [{}]", sender);
                    ...
                    if (CollectionUtils.isNotEmpty(directedPresences)) {
                        metrics.addTime(METRIC_GET_DIRECTED_PRESENCES_CACHE_TIMER, System.currentTimeMillis() - getStartTime, SI.MILLI(SI.SECOND));
                    }
                    metrics.addCount(METRIC_DIRECTED_PRESENCES_CACHE_SIZE, directedPresencesCache.size(), Unit.ONE);

                    Log.info("Presence type is {}", update.getType());
                    metrics.addCount(METRIC_PRESENCE_TYPE_PREFIX + update.getType() + METRIC_COUNT_SUFFIX, 1, Unit.ONE);  /** SINGLE LINE amazon ***/
                    if (Presence.Type.unavailable.equals(update.getType())) {
                        if (directedPresences != null) {
                            ...
                            if (directedPresences.isEmpty()) {
                                ...
                                // presences is empty
                                Log.info("Remove unavailable directedPresences for sender: [{}]", sender);
                                ...
                                metrics.addTime(METRIC_REMOVE_DIRECTED_PRESENCES_CACHE_TIMER, System.currentTimeMillis() - removeStartTime, SI.MILLI(SI.SECOND));
                            }
                            else {
                                Log.info("Put unavailable directedPresences for sender: [{}]", sender);
                                ...
                                metrics.addTime(METRIC_PUT_DIRECTED_PRESENCES_CACHE_TIMER, System.currentTimeMillis() - putStartTime, SI.MILLI(SI.SECOND));
                            }
                        }
                        metrics.addTime(METRIC_UNAVAILABLE_PRESENCE_TIMER, System.currentTimeMillis() - startTime, SI.MILLI(SI.SECOND));  /** SINGLE LINE amazon ***/
                    } else {
                        ...
                        Log.info("Put available directedPresences for sender: [{}]", sender);
                        long putStartTime = System.currentTimeMillis();
                        directedPresencesCache.put(sender, directedPresences);
                        localDirectedPresences.put(sender, directedPresences);
                        metrics.addTime(METRIC_PUT_DIRECTED_PRESENCES_CACHE_TIMER, System.currentTimeMillis() - putStartTime, SI.MILLI(SI.SECOND));
                        metrics.addTime(METRIC_PRESENCE_TYPE_PREFIX + update.getType() + METRIC_TIMER_SUFFIX, System.currentTimeMillis() - startTime, SI.MILLI(SI.SECOND));  /** SINGLE LINE amazon ***/
                    }
                } finally {
                    lock.unlock();
                    metrics.addTime(METRIC_DIRECTED_PRESENCE_ROSTER_KEEP_TRACK_TIMER, System.currentTimeMillis() - startTime, SI.MILLI(SI.SECOND));  /** SINGLE LINE amazon ***/
                }
            }
        }
    }
