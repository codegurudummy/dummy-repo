...
public class IQRouter extends BasicModule {
    ...
    /*** BEGIN amazon ***/
    private static final String METRIC_ROUTE_MESSAGE_OPERATION = "IQRouter.RouteMessage";
    private static final String METRIC_ROUTE_MESSAGE_PREFIX = METRIC_ROUTE_MESSAGE_OPERATION + ".";
    private static final String METRIC_FAILURE_PREFIX = METRIC_ROUTE_MESSAGE_PREFIX + "Failure.";

    // Informational metrics
    private static final String METRIC_ROUTE_MESSAGE_MULTICAST = METRIC_ROUTE_MESSAGE_PREFIX + "Multicast";
    private static final String METRIC_ROUTE_MESSAGE_COMPONENT_FOUND = METRIC_ROUTE_MESSAGE_PREFIX +
            "ComponentFound";
    private static final String REQUEST_FAILED = METRIC_ROUTE_MESSAGE_PREFIX + "RoutingFailed";
    private static final String REQUEST_SUCCEEDED = METRIC_ROUTE_MESSAGE_PREFIX + "RoutingSucceeded";

    // Failure metrics while attempting to route IQ packet
    private static final String METRIC_ROUTE_MESSAGE_FAILURE_BAD_REQUEST = METRIC_FAILURE_PREFIX + "BadRequest";
    private static final String METRIC_ROUTE_MESSAGE_FAILURE_INTERNAL_SERVER_ERROR = METRIC_FAILURE_PREFIX +
            "InternalServerError";
    private static final String METRIC_ROUTE_MESSAGE_FAILURE_LOCAL_COMMUNICATION_BLOCKED = METRIC_FAILURE_PREFIX +
            "LocalCommunicationBlocked";
    private static final String METRIC_ROUTE_MESSAGE_FAILURE_NOT_ACCEPTABLE = METRIC_FAILURE_PREFIX + "NotAcceptable";
    private static final String METRIC_ROUTE_MESSAGE_FAILURE_NOT_AUTHORIZED = METRIC_FAILURE_PREFIX +
            "NotAuthorized";
    private static final String METRIC_ROUTE_MESSAGE_FAILURE_NULL_MESSAGE = METRIC_FAILURE_PREFIX + "NullMessage";
    private static final String METRIC_ROUTE_MESSAGE_FAILURE_PACKET_REJECTED = METRIC_FAILURE_PREFIX +
            "PacketRejected";
    private static final String METRIC_ROUTE_MESSAGE_FAILURE_PROCESSING_PACKET = METRIC_FAILURE_PREFIX +
            "ProcessingPacket";
    private static final String METRIC_ROUTE_MESSAGE_FAILURE_RESPONSE_TO_IQ_ERROR = METRIC_FAILURE_PREFIX +
            "ErrorResponseToIQError";
    private static final String METRIC_ROUTE_MESSAGE_FAILURE_SESSION_NULL = METRIC_FAILURE_PREFIX + "SessionNull";
    private static final String METRIC_ROUTE_MESSAGE_FAILURE_UNKNOWN_PACKET = METRIC_FAILURE_PREFIX + "UnknownPacket";

    // Metric to record for routeFailed method call
    private static final String METRIC_ROUTING_FAILED_SEND_ERROR_CALL_COUNT = "IORouter.RoutingFailed.SendErrorCallCount";
    /*** END amazon ***/
...
/*** BEGIN amazon ***/
        ...
        final Metrics metric = OpenfireMetricsHelper.getInstance().initializeOperation(
                METRIC_ROUTE_MESSAGE_OPERATION, startTime);

        ClientSession session = null;
        Element childElement = null;
        try {
            if (packet == null) {
                metric.addCount(METRIC_ROUTE_MESSAGE_FAILURE_NULL_MESSAGE, 1, Unit.ONE);
                throw new NullPointerException();
            }
            JID sender = packet.getFrom();
            session = sessionManager.getSession(sender);
            childElement = packet.getChildElement(); // may be null
/*** END amazon ***/
            ...
            if (session != null && to != null && session.getStatus() == Session.STATUS_CONNECTED &&
                ...
                Log.warn("User tried to authenticate with this server using an unknown receipient: " +
                        packet); //ONE LINE AMAZON
                metric.addCount(METRIC_ROUTE_MESSAGE_FAILURE_BAD_REQUEST, 1, Unit.ONE); /*** SINGLE LINE amazon ***/
            }
                ...
                isSuccessful = handle(packet, metric); /*** SINGLE LINE amazon ***/
                ...
                session.process(reply);
                metric.addCount(METRIC_ROUTE_MESSAGE_FAILURE_NOT_AUTHORIZED, 1, Unit.ONE); /*** SINGLE LINE amazon ***/
            }
...
/*** BEGIN amazon ***/
            isSuccessful = false;
            metric.addCount(METRIC_ROUTE_MESSAGE_FAILURE_PACKET_REJECTED, 1, Unit.ONE);
        } finally {
            final long endTime = System.currentTimeMillis();
            metric.addTime((isSuccessful ? REQUEST_SUCCEEDED : REQUEST_FAILED), endTime - startTime,
                    SI.MILLI(SI.SECOND));
...
/*** BEGIN amazon ***/
    /**
     * Handle the IQ packet
     *
     * @param packet packet to be handled
     * @param metric Metric object for instrumentation. Can be null
     * @return True, if the packet was handled successfully. False, in case of failures.
     */
    private boolean handle(final IQ packet, final Metrics metric) {
...
/*** END amazon ***/
        ...
        if (recipientJID != null && recipientJID.getNode() == null &&
            ...
            if (childElement != null && childElement.element("addresses") != null) {
                ...
                /*** BEGIN amazon ***/
                if (metric != null) {
                    metric.addCount(METRIC_ROUTE_MESSAGE_MULTICAST, 1, Unit.ONE);
                }
                return isSuccessful;
                /*** END amazon ***/
            }
        ...
        if (packet.getID() != null && (IQ.Type.result == packet.getType() || IQ.Type.error == packet.getType())) {
            ...
            if (iqResultListener != null) {
                ...
                if (iqResultListener != null) {
                    ...
                    /*** BEGIN amazon ***/
                    Log.error("Error processing answer of remote entity. Answer: " + packet, e);
                        isSuccessful = false;
                        if (metric != null) {
                            metric.addCount(METRIC_ROUTE_MESSAGE_FAILURE_PROCESSING_PACKET, 1, Unit.ONE);
                        }
                    }
                    return isSuccessful;
                    /*** END amazon ***/
        ...
        try {
            ...
            if (recipientJID != null &&
                ...
                /*** BEGIN amazon ***/
                if (metric != null) {
                    metric.addCount(METRIC_ROUTE_MESSAGE_COMPONENT_FOUND, 1, Unit.ONE);
                }
                return isSuccessful;
                /*** END amazon ***/
            }
           ...
           if (isLocalServer(recipientJID)) {
                ...
                if (namespace == null) {
                    if (packet.getType() != IQ.Type.result && packet.getType() != IQ.Type.error) {
                        ...
                        isSuccessful = false;
                        if (metric != null) {
                            metric.addCount(METRIC_ROUTE_MESSAGE_FAILURE_UNKNOWN_PACKET, 1, Unit.ONE);
                        }
                        /*** END amazon ***/
                    }
                }
                    ...
                    if (recipientJID != null && userManager.isRegisteredUser(recipientJID.getNode())) {
                        ...
                        if (list != null && list.shouldBlockPacket(packet)) {
                            isSuccessful = false; /*** SINGLE LINE amazon ***/
                            // Communication is blocked
                            if (IQ.Type.set == packet.getType() || IQ.Type.get == packet.getType()) {
                                // Answer that the service is unavailable
                            /*** BEGIN amazon ***/
                                sendErrorPacket(packet, PacketError.Condition.service_unavailable, metric);
                            } else if (metric != null) {
                                metric.addCount(METRIC_ROUTE_MESSAGE_FAILURE_LOCAL_COMMUNICATION_BLOCKED, 1, Unit.ONE);
                            }
                            return isSuccessful;
                            /*** END amazon ***/
                        }
                    ...
                    if (handler == null) {
                        ...
                        if (recipientJID == null) {
                            ...
                            sendErrorPacket(packet, PacketError.Condition.service_unavailable, metric); //*** amazon ONE_LINE ***
                        }
                        else if (recipientJID.getNode() == null ||
                                "".equals(recipientJID.getNode())) {
                            // Answer an error if JID is of the form <domain>
                            sendErrorPacket(packet, PacketError.Condition.feature_not_implemented, metric); //*** amazon ONE_LINE ***
                        }
                        else {
                ...
                if (session instanceof LocalClientSession) {
                    ...
                    if (!((LocalClientSession) session).canProcess(dummyIQ)) {
                        ...
                        isSuccessful = false;
                        if (metric != null) {
                            metric.addCount(METRIC_ROUTE_MESSAGE_FAILURE_NOT_ACCEPTABLE, 1, Unit.ONE);
                        }
                        /*** END amazon ***/
                    }
                }
            ...
            isSuccessful = false;
            if (metric != null) {
                metric.addCount((session != null ? METRIC_ROUTE_MESSAGE_FAILURE_INTERNAL_SERVER_ERROR :
                        METRIC_ROUTE_MESSAGE_FAILURE_SESSION_NULL), 1, Unit.ONE);
            }
            /*** END amazon ***/
        }
        return isSuccessful; /*** SINGLE LINE amazon ***/
    }

    private void sendErrorPacket(IQ originalPacket, PacketError.Condition condition, final Metrics
            metric) { /*** SINGLE LINE amazon ***/
        if (IQ.Type.error == originalPacket.getType()) {
            /*** BEGIN amazon ***/
            if (metric != null) {
                metric.addCount(METRIC_ROUTE_MESSAGE_FAILURE_RESPONSE_TO_IQ_ERROR, 1, Unit.ONE);
            }
            /*** END amazon ***/
        ...
        /*** BEGIN amazon ***/
        if (metric != null) {
            metric.addCount(METRIC_FAILURE_PREFIX + condition.toString(), 1, Unit.ONE);
        }
        /*** END amazon ***/
        ...
        if (serverName.equals(originalPacket.getFrom().toString())) {
            ...
            handle(reply, metric); /*** SINGLE LINE amazon ***/
            return;
        }
    ...
    {
        Log.debug("IQ sent to unreachable address: " + packet); // ONE LINE AMAZON
        ...
        {
            /*** BEGIN amazon ***/
            // adding a metric to track if, and how frequently, this is called. if this is called frequently,
            // it might be worth instrumenting the method from routing table.
            final long startTime = System.currentTimeMillis();
            final Metrics metric = OpenfireMetricsHelper.getInstance().initializeOperation(
                    METRIC_ROUTE_MESSAGE_OPERATION, startTime);
            metric.addCount(METRIC_ROUTING_FAILED_SEND_ERROR_CALL_COUNT, 1, Unit.ONE);
            sendErrorPacket( iq, PacketError.Condition.service_unavailable, metric );
            OpenfireMetricsHelper.getInstance().finalizeOperation(metric, startTime);
            /*** END amazon ***/
        }
    }
    ...
    /*** BEGIN amazon ***/
    /**
     * Set the session manager. Testing purpose only.
     *
     * @param sessionManager Session manager to set
     */
    /* package */ void setSessionManager(final SessionManager sessionManager) {
        this.sessionManager = sessionManager;
    }

    /**
     * Set the Server name. Testing Purpose only.
     *
     * @param serverName Server name to set
     */
    /* package */ void setServerName(final String serverName) {
        this.serverName = serverName;
    }

    /**
     * Set the user manager. Testing purpose only.
     * @param userManager UserManager to set.
     */
    /* package */ void setUserManager(final UserManager userManager) {
        this.userManager = userManager;
    }

    /**
     * Set the Routing table. Testing Purpose only.
     *
     * @param routingTable Routing table to set.
     */
    /* package */ void setRoutingTable(final RoutingTable routingTable) {
        this.routingTable = routingTable;
    }

    /**
     * Set the Multicast router. Testing purpose only.
     *
     * @param multicastRouter Multicast router to set
     */
    /* package */ void setMulticastRouter(final MulticastRouter multicastRouter) {
        this.multicastRouter = multicastRouter;
    }
    /*** END amazon ***/
}