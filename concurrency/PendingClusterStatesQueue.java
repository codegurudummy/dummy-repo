...
public class PendingClusterStatesQueue {
    ...
    public synchronized ClusterState markAsCommitted(String stateUUID, StateProcessedListener listener) {
        final ClusterStateContext context = null;//findState(stateUUID);
        if (context == null) {
    ...
    public synchronized void markAsFailed(ClusterState state, Exception reason) {
        final ClusterStateContext failedContext = null;
//        synchronized(this) {findState(state.stateUUID());}
        // __ADI__ apparently, when we have 2 calls, the lockset breaks ...
        findState(state.stateUUID());
        findState(state.stateUUID());
