...
import com.amazon.campfire.roster.RosterUtils;
...
public class Roster implements Cacheable, Externalizable {
    ...
    /** BEGIN Amazon **/
    private static final String METRIC_ROSTER_GET_USER_TIMER = "Roster.GetUser.Timer";
    private static final String METRIC_ROSTER_PRESENCE_BROADCAST_TIMER = "Roster.PresenceBroadcast.Timer";
    private static final String METRIC_ROSTER_BROADCAST_COUNT = "Roster.PresenceBroadcast.Count";
    private static final String METRIC_CREATE_ROSTER = "Roster.Create";
    private static final String METRIC_ROSTER_ITEMS_COUNT = "Roster.RosterItems.Count";
    /** END Amazon **/
    ...
    Roster(String username) {
        ...
        final Metrics metric = OpenfireMetricsHelper.getInstance().initializeOperation(METRIC_CREATE_ROSTER, startTime);
        /** END Amazon */
        try {
            ...
            metric.addCount(METRIC_ROSTER_ITEMS_COUNT, 1, Unit.ONE); /** SINGLE LINE amazon ***/
...
// *** BEGIN amazon ***
        rosterItem = RosterUtils.addUserInfo(rosterItem, METRIC_ROSTER_GET_USER_TIMER);
// *** END amazon ***
    ...
    public org.xmpp.packet.Roster getReset() {
        ...
        for (RosterItem item : rosterItems.values()) {
            ...
            if (item.getSubStatus() != RosterItem.SUB_NONE ||
                    item.getRecvStatus() != RosterItem.RECV_SUBSCRIBE && !isSubscriptionRejected(item)) {
// *** BEGIN amazon ***
                roster.addItem(item.getJid(), item.getNickname(), ask, sub, groups, item.getAmazonCustomerId());
// *** END amazon ***
            }
        }
        return roster;
     ...
     */
    public void broadcastPresence(Presence packet, final Metrics metric) {
        ...
        final long bcastStartTime = System.currentTimeMillis();
        int bcastCount = 0;
        for (RosterItem item : rosterItems.values()) {
        ...
        metric.addTime(METRIC_ROSTER_PRESENCE_BROADCAST_TIMER, System.currentTimeMillis() - bcastStartTime, SI.MILLI(SI.SECOND));
        metric.addCount(METRIC_ROSTER_BROADCAST_COUNT, bcastCount, Unit.ONE);
        if (from != null) {
